...
- (NSArray *)fetchAllLocations:(NSManagedObjectContext *)context{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"MoodSpot"];
    ...
    for(int i = 0; i < [locations count]; i++){
        locationNames[i] = ((MoodSpot *)locations[i]).name;
    }
    return locationNames;
}

- (NSArray *)fetchAllActivities:(NSManagedObjectContext *)context{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"MoodActivity"];
    request.fetchBatchSize = 20;
    request.fetchLimit = 100;
    NSError *error;
    ...
    for(int i = 0; i < [activities count]; i++){
        activityNames[i] = ((MoodActivity *)activities[i]).name;
    }
    return activityNames;
}

- (NSArray *)fetchAllNearbyPeople:(NSManagedObjectContext *)context{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"MoodPerson"];
    request.fetchBatchSize = 20;
    request.fetchLimit = 100;
    NSError *error;
    ...
    for(int i = 0; i < [people count]; i++){
        names[i] = ((MoodPerson *)people[i]).name;
    }
    return names;
}
